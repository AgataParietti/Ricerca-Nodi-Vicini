...
public class Embeddings {
    ...
    public Double exec(NodeValue nv1, NodeValue nv2, HashMap<String, float[]> coordinates) {
        ...
        return cosineDist(node1Name, node2Name, coordinates);
    }
    ...
    private double cosineDist(String node1Name, String node2Name, HashMap<String, float[]> coordinates) {
        if (!coordinates.containsKey(node1Name) || !coordinates.containsKey(node2Name)) {
            return Double.POSITIVE_INFINITY;
        }
        float [] vectorA = coordinates.get(node1Name);
        float [] vectorB = coordinates.get(node2Name);
